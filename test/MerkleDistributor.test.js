...
const { upgradeContracts, upgradeRewardEthToken } = require('../deployments');
...
contract(
  'Merkle Distributor',
  ([beneficiary, anyone, vault, ...otherAccounts]) => {
    ...
    describe('claim', () => {
      ...
      it('cannot claim twice', async () => {
        await addStakedEthToken(stakedEthToken, anyone, ether('100'));
      ...
      it('can claim reward tokens', async () => {
        ...
        for (const [
          account,
          { index, proof, tokens, amounts },
        ] of Object.entries(merkleProofs)) {
      ...
      describe('claiming within the same block', () => {
        ...
        it('cannot claim before merkle root update in the same block', async () => {
          ...
          await rewardEthToken.updateTotalRewards(rewardsDelta, {
            from: vault,
          });
          await expectRevert(
